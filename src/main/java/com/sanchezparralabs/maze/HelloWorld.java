...
public class HelloWorld extends Application {
    ...
    final Group wallsGroup = new Group();
    ...
    private void buildWalls() {
        ...
        grayMaterial.setDiffuseColor(Color.GREY);
        grayMaterial.setSpecularColor(Color.LIGHTGRAY);

        List<PhongMaterial> materials = Arrays.asList(redMaterial, greenMaterial, blueMaterial);

        MazeBuilder builder = new MazeBuilder();
        char[][] maze = builder.build(10, 10);
        builder.print(maze);
        for(int y = 0; y <= 10; y++) {
            for(int x = 0; x <= 10; x++) {
                char c = maze[y][x];
                ...
                wallsGroup.getChildren().add(box);
                box.setTranslateX((x - 5) * 30);
                box.setTranslateZ((y - 5) * 30);
            }
        }
        world.getChildren().addAll(wallsGroup);
    }
